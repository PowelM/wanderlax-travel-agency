...
import { getHotelById, updateHotel } from '@/app/actions/hotelActions';
...
export default function EditHotelPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { id: hotelId } = React.use(params);
      ...
      getHotelById(hotelId).then(data => {
        if (data) {
          setForm({
            ...
            starRating: data.starRating || 'UNRATED',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await updateHotel(hotelId, cleanedForm as any);
    setIsSaving(false);
    ...
    return (
      <div className="flex h-screen w-full bg-[#120d0d]">
        ...
        <main className="flex-1 flex flex-col items-center justify-center">
            ...
            <p className="text-white mt-4">Loading Hotel Details...</p>
        </main>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-screen w-full bg-[#120d0d]">
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden">
        <AdminHeader title="Edit Hotel" description={`Update details for ${form.name}`}>
          <div className="flex gap-3">
            ...
            <button form="edit-hotel-form" type="submit" disabled={isSaving} className="px-6 py-2 rounded-lg bg-primary text-white text-sm font-bold hover:bg-red-700 transition-all disabled:opacity-50 shadow-lg shadow-primary/30">
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8">
          <form id="edit-hotel-form" onSubmit={handleSubmit} className="max-w-4xl mx-auto space-y-8">
            {error && (
