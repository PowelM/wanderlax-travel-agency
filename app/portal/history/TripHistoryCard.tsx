...
export function TripHistoryCard({ 
  ...
  if (booking.serviceType === 'TOUR_PACKAGE' && booking.tourBooking) {
    ...
    title = `${booking.carHireBooking.car?.make} ${booking.carHireBooking.car?.model} Rental`;
    location = booking.carHireBooking.pickupLocation;
  ...
  const handleDownloadPDF = () => {
    try {
      ...
      // 1. Header Section
      // Top accent line
      doc.setFillColor(198, 16, 16); // primary color #c61010
      doc.rect(0, 0, pageWidth, 8, 'F');
      
      // Wanderlux Text Logo
      doc.setFont("helvetica", "bold");
      doc.setTextColor(198, 16, 16); // primary color
      doc.setFontSize(28);
      ...
      doc.setFont("helvetica", "normal");
      doc.text(`#${booking.id.substring(0, 8).toUpperCase()}`, margin + 40, 50);
      
      doc.setFont("helvetica", "bold");
      doc.text("Status:", pageWidth - margin - 40, 50);
      
      doc.setFont("helvetica", "normal");
      doc.setTextColor(isCompleted ? 16 : 37, isCompleted ? 185 : 99, isCompleted ? 129 : 235); // Emerald vs Blue
      ...
      addRow("Service Type:", booking.serviceType.replace('_', ' '), currentY);
      currentY += 10;
  ...
  const submitReview = async () => {
    ...
    // In a real app, this would be a fetch call or Server Action to save the review to the DB
    try {
  ...
  return (
    <article className="group relative flex flex-col gap-0 bg-white dark:bg-card-dark rounded-2xl overflow-hidden border border-slate-200 dark:border-border-dark shadow-xl hover:border-primary/40 transition-all duration-500 print:shadow-none print:border-none">
          ...
          <div>
            ...
            <div className="space-y-2 mb-6">
              ...
              <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400">
                ...
                <span className="text-sm font-medium">Total: <span className="text-slate-900 dark:text-white font-bold">${Number(booking.finalAmount).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span></span>
              </div>
            </div>
          </div>
      ...
      {isReviewing && (
        <div className="border-t border-slate-100 dark:border-border-dark bg-slate-50 dark:bg-background-dark p-8 animate-fade-in-up">
          <h4 className="text-lg font-bold text-slate-900 dark:text-white mb-4 italic uppercase">How was your trip?</h4>
