      ...
      <main className="relative z-10 w-full max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 py-6 flex flex-col gap-10">
        ...
        <div className="relative z-20 -mt-24 mx-4 md:mx-10 bg-surface-dark border border-border-dark p-6 rounded-xl shadow-2xl backdrop-blur-md">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="space-y-2">
              <label className="text-xs font-semibold text-text-muted uppercase tracking-wider">Pickup Location</label>
              <div className="relative">
                <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-500">location_on</span>
                <input className="w-full bg-background-dark border border-border-dark rounded-lg py-3 pl-10 pr-4 text-white placeholder-slate-600 focus:ring-1 focus:ring-primary focus:border-primary transition-colors outline-none" placeholder="City, Airport, or Address" type="text"/>
              </div>
            </div>
            <div className="space-y-2">
              <label className="text-xs font-semibold text-text-muted uppercase tracking-wider">Drop-off Location</label>
              <div className="relative">
                <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-500">flag</span>
                <input className="w-full bg-background-dark border border-border-dark rounded-lg py-3 pl-10 pr-4 text-white placeholder-slate-600 focus:ring-1 focus:ring-primary focus:border-primary transition-colors outline-none" placeholder="Return to same location" type="text"/>
              </div>
            </div>
            <div className="space-y-2">
              <label className="text-xs font-semibold text-text-muted uppercase tracking-wider">Pickup Date</label>
              <div className="relative">
                <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-500">calendar_today</span>
                <input className="w-full bg-background-dark border border-border-dark rounded-lg py-3 pl-10 pr-4 text-white focus:ring-1 focus:ring-primary focus:border-primary transition-colors outline-none [color-scheme:dark]" type="date"/>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 py-8 border-b border-border-dark/50">
          <div className="flex items-center gap-4 p-4 rounded-lg bg-surface-dark/50 border border-border-dark/50">
            ...
            <div>
              ...
              <p className="text-sm text-text-muted">Comprehensive coverage included</p>
            </div>
          </div>
          <div className="flex items-center gap-4 p-4 rounded-lg bg-surface-dark/50 border border-border-dark/50">
            <div className="size-12 rounded-full bg-primary/10 flex items-center justify-center text-primary">
            ...
            <div>
              ...
              <p className="text-sm text-text-muted">Never get lost on your journey</p>
            </div>
          </div>
          <div className="flex items-center gap-4 p-4 rounded-lg bg-surface-dark/50 border border-border-dark/50">
            <div className="size-12 rounded-full bg-primary/10 flex items-center justify-center text-primary">
            ...
            <div>
              ...
              <p className="text-sm text-text-muted">Concierge assistance anytime</p>
            </div>
          </div>
        </div>
        ...
        <section className="py-4">
          <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-4">
            <div>
              ...
              <p className="text-text-muted">Choose from our top-tier luxury vehicles.</p>
            </div>
            <div className="hidden md:flex gap-2">
              {["Sports", "SUV", "Sedan", "Convertible"].map(category => (
                <button 
                  key={category}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {cars.map(car => (
              <div key={car.id} className="group bg-surface-dark rounded-xl overflow-hidden border border-border-dark hover:border-primary/50 transition-all hover:shadow-2xl hover:shadow-primary/5 flex flex-col">
                <div className="relative h-56 overflow-hidden">
                  ...
                  <div className="absolute top-4 right-4 bg-black/60 backdrop-blur-md text-white text-xs font-bold px-3 py-1 rounded-full border border-white/10">{car.type}</div>
                </div>
                <div className="p-6 flex flex-1 flex-col">
                  <h3 className="text-xl font-bold text-white mb-1 group-hover:text-primary transition-colors">{car.name}</h3>
                  <p className="text-text-muted text-sm mb-4">{car.description}</p>
          ...
          <div className="mt-10 flex justify-center">
            <button className="text-slate-300 hover:text-primary font-medium text-sm flex items-center gap-2 border-b border-transparent hover:border-primary pb-1 transition-all group">
              Show more vehicles
              <span className="material-symbols-outlined text-base group-hover:translate-y-1 transition-transform">expand_more</span>
            </button>
          </div>
        </section>
        ...
        <section className="py-12 border-t border-border-dark/50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
            ...
            {[
              { step: "1", title: "Choose Your Dates", desc: "Select your pickup and return dates. We offer flexible booking options.", icon: "calendar_month" },
              { step: "2", title: "Select Your Vehicle", desc: "Browse our premium fleet and choose the car that matches your style.", icon: "directions_car" },
              { step: "3", title: "Enjoy the Ride", desc: "Pick up your keys or get it delivered. The road is yours to conquer.", icon: "key" }
            ].map(item => (
